...
#Script to Fetch a cPanel account's Data(Website, DB, Mails) and Logs (SSH, FTP, Exim, Domain access, cPanel, Mailbox). - O/P will be a downloadable link of a remote server.
clear
passwd="xxxxxxxxx"
m=0;
em=0;
...
then
        ...
        info1=`date +"%H:%M%p_%d-%m-%y"_$dom`_Data.txt
        touch $info1
        curl --insecure --user root:$passwd -T $info1 sftp://xxx.x.x.xxx/usr/local/apache/htdocs/abuse_script_exe_logs/ &> /dev/null
        rm -fv $info1 &> /dev/null
        echo -e "\n$vio++++++++++++++++++DATABASE++++++++++++++++++$res\n"
...

{
	...
	echo -e "\n$(tput setaf 1)\n############################################\n\n DO NOT ENTER PASSWORD!!! IT WILL BE PASSED AUTOMATICALLY IN 5...4...3...2...1...!\n\n############################################\n $(tput sgr 0) \n"
	expect - << EOF
	spawn rsync -ae "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no" abuse_data_$dom.tar.gz root@xxx.x.x.xxx:/usr/local/apache/htdocs/
	expect "Password:"
	send "xxxxxxx"
...
EOF
	...
	echo -e "\n$cya Transfer completed!$res \n";
	rm -rf abuse_data_$dom.tar.gz
	rm -fv abuse.sh &> /dev/null
	echo -e "\n$(tput setaf 4)<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>\n\n$res Downloadable link: $(tput sgr 2)http://xxx.x.x.xxx/abuse_data_$dom.tar.gz $res\n\n$(tput setaf 4)<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>$res\n";
...
logs()
        {
			...
			info=`date +"%H:%M%p_%d-%m-%y"_$dom1`_Logs.txt
			touch $info
			curl --insecure --user root:$passwd -T $info sftp://xxx.x.x.xxx/usr/local/apache/htdocs/abuse_script_exe_logs/ &> /dev/null
                        rm -fv $info &> /dev/null
			final
                        exit
                     fi
...
final()
        {
                ...
                du -sch $dom1.zip
                echo -e "\n$cya Transferring zip file to demomonkey server... $res\n"
		echo -e "\n$red\n############################################\n\n DO NOT ENTER PASSWORD!!! IT WILL BE PASSED AUTOMATICALLY IN 5...4...3...2...1...!\n\n############################################\n $res \n"
expect - << EOF
spawn rsync -ae "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no" $dom1.zip root@xxx.x.x.xxx:/usr/local/apache/htdocs/
expect "Password:"
send "xxxxxxx"
send_user "\n\n Transferring to demomonkey...Hold on!\n"
if [catch wait] {
    		puts "rsync failed"
		     exit 1
}
EOF
		...
		echo -e "\n$cya Transfer is Completed!$res \n"
                echo -e "\n$cya Removing log files created from this server... $res \n"
                rm -fv *.txt *.zip &> /dev/null
                echo -e "\n$(tput setaf 4)<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>\n\n$res Downloadable link: $(tput sgr 2)http://xxx.x.x.xxx/$dom1.zip $res\n\n$(tput setaf 4)<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>$res\n";
		rm -fv ../abuse.sh &> /dev/null
        }
